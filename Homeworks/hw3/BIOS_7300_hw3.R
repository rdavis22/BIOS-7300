...
library(survival)
library(KMsurv)
hw3data<-read.table(file=file.choose(), header=T, sep="") #read in "Comparison of prostate cancer patients.dat"
...
# library(KMsurv)
# library(survMisc)
part34data<-read.table(file=file.choose(), header=T, sep="") #read in "Time to death...transplants.dat"
attach(part34data)
agecat<- cut(age, c(min(age), 50, 60, max(age))) #cuts from the "Time to death...transplants.dat" document
...
test.num <- zz %*% coef(trend_3) #numerator "Z" of Trend test statistic
test.var <- zz %*% trend_3$var %*% zz #Variance of test
lr_trend<-test.num/sqrt(test.var) #Test statistic with standard normal distribution [i.e.N~(0,1)] ]
print(lr_trend)
